...
async def on_new_private_message(event):
    ...
    if event.chat_id == 5656828413:
        await event.client.send_message(chat, "**⪼ اطـلق هـلاو مطـوري الغـالي فينوم اننـي محظـوظ لقدومـك الـي 🙈♥️𓆰*")
        reason = "**يجب عليك الترحيب بمطور السورس **"
    if event.chat_id == 5688682765:
        await event.client.send_message(chat, "**⪼ اطـلق هـلاو مطوري الغـالي فينوم اننـي محظـوظ لقدومـك الـي 🙈♥️𓆰*")
        reason = "**يجب عليك الترحيب بمطور السورس **"
...
async def on_plug_in_callback_query_handler(event):
    ...
    try:
        ...
        PM_WARNS = {}  # ترجمه وكتابة فريق فينوم
    if str(event.query.user_id) in PM_WARNS:
        del PM_WARNS[str(event.query.user_id)]
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
...
@sbb_b.ar_cmd(pattern="الحماية (تشغيل|تعطيل)$")  # ترجمه وكتابة فريق فينوم
...
async def pmpermit_on(event):
    input_str = event.pattern_match.group(1)
    if input_str == "تعطيل":
        if gvarstatus("pmmenu") is None:  # ترجمه وكتابة فريق فينوم
...
async def approve_p_m(event):
    ...
    if len(approved_users) > 0:
        for user in approved_users:
            APPROVED_PMs += f"• 𓆰 𝗦𝗢𝗨𝗥𝗖𝗘 𝗩𝗘𝗡𝗢𝗠 𓆪\n 𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻"
    else:
        APPROVED_PMs = "انت لم توافق على اي شخص بالاصل ️"
    await edit_or_reply(
        event,
        APPROVED_PMs,
        file_name="قائمة الحماية فينوم.txt",
